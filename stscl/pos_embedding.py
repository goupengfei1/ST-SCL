...
class SpaEmbedding(nn.Module):
    ...
    def __init__(self, node_dim, num_nodes, root_path, if_rel=False):
        super(SpaEmbedding, self).__init__()
        self.if_rel = if_rel
        ...
        if self.if_rel is False:
            ...
            self.embedding = nn.Embedding(node_dim, node_dim)

    def forward(self, x):
        B, C, N, _ = x.shape

        pos = self.node_pos.to(x.device)
        node_emb = self.embedding(pos).repeat(B, C, 1, 1)
        return x + node_emb
