...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def __init__(self, cfg: dict):
        ...
        self.con = cfg["MODEL"]["PARAM"].get("if_con", True)
        ...
        if self.con:
            self.supervised_loss_ratio = cfg["TRAIN"]["SUPERVISED_LOSS_RATIO"]

        # define metric
        # self.train_metrics = cfg.get("METRICS",{"", "MAE": masked_mae, "RMSE": masked_rmse, "MAPE": masked_mape, "WAPE": masked_wape, "MSE": masked_mse})
    ...
    def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]) -> torch.Tensor:
        ...
        loss = self.metric_forward(self.loss, [forward_return[0], forward_return[1]])
        ...
        if self.con:
            supervised_component = self.supervised_loss_ratio * loss
            con_component = (1 - self.supervised_loss_ratio) * cl_loss
            loss = supervised_component + con_component
