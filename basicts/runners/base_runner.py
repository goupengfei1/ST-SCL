...
class BaseRunner(Runner):
    ...
    def __init__(self, cfg: dict):
        ...
        self.patience = cfg.get("EARLY_STOP_PATIENCE", 3)  # Epochs to tolerate without improvement
        self.early_stop_counter = 0
        self.prev_val_metric = float('inf')  # Assuming smaller is better (e.g., Loss, MAE)
    ...
    def on_epoch_end(self, epoch: int):
        ...
        if self.early_stop_triggered:
            self.logger.info("Early Stopping Triggered! Exiting training loop ...")
            sys.exit(0)
        # save model
    ...
    def on_validating_end(self, train_epoch: Optional[int]):
        ...
        if train_epoch is not None:
            # Replace with your actual monitoring metric name, e.g., "val_loss" or "val_MAE"
            monitor_metric = "val_MAE"
            self.save_best_model(train_epoch, "val_MAE", greater_best=False)

            # Get the average metric for the current validation epoch from meter_pool
            current_metric = self.meter_pool.get_avg(monitor_metric)

            # Compare ONLY with the previous epoch's metric
            if current_metric < self.prev_val_metric:
                self.early_stop_counter = 0
                self.logger.info(f"Metric improved from {self.prev_val_metric:.4f} to {current_metric:.4f}")
            else:
                self.early_stop_counter += 1
                self.logger.info(
                    f"No improvement from previous epoch. Early stopping counter: {self.early_stop_counter} / {self.patience}")

                if self.early_stop_counter >= self.patience:
                    self.logger.info(
                        f"Metric degraded or stalled for {self.patience} consecutive epochs. Early stopping triggered!")
                    self.early_stop_triggered = True

            # Update the previous metric for the next epoch's comparison
            self.prev_val_metric = current_metric
